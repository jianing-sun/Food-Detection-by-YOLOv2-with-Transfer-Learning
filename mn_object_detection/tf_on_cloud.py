...
def train(model):
    ...
    checkpoint = ModelCheckpoint('transferLearning_valloss.h5',
                                 monitor='val_loss',
                                 verbose=1,
                                 save_best_only=True,
                                 mode='min',
    ...
    tensorboard = TensorBoard(log_dir=os.path.expanduser('~/mess/') + 'transferLearning_net' + '_' + str(tb_counter),
                              histogram_freq=0,
                              write_graph=True,
                              write_images=False)
...
if __name__ == '__main__':
    ...
    ANCHORS = []
    with open('../UECFOOD100_JS/generated_anchors/anchors_5.txt', 'r') as anchor_file:
